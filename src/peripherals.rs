use crate::{
    apu::Apu, bootrom::Bootrom, cartridge::Cartridge, cpu::interrupts::Interrupts, hram::HRam,
    joypad::Joypad, ppu::Ppu, timer::Timer, wram::WRam,
};
...
impl Peripherals {
    ...
    pub fn read(&self, interrupts: &Interrupts, addr: u16) -> u8 {
        match addr {
            0x0000..=0x00FF if self.bootrom.is_active() => self.bootrom.read(addr),
